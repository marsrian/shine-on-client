...
const NecklaceCollection = () => {
  ...
  const filteredData = data.filter((item) => item.category === category);
    ...
    return (
        <div>
      <h1 className="text-3xl font-bold text-gray-800 text-center mb-3">Necklaces Collection</h1>
      {loading ? (
          <p className="text-center text-3xl font-bold text-red-400">
            ...
            {filteredData.map((necklace) => (
              <NecklaceAllCollection key={necklace._id} necklace={necklace}></NecklaceAllCollection>
            ))}
          </div>
        )}
    </div>
